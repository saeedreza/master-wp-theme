...
add_action('after_setup_theme','master_start', 16);
...
function master_start() {
    ...
    add_action( 'widgets_init', 'master_register_sidebars' );
...
return '<a class="excerpt-read-more" href="'. get_permalink($post->ID) . '" title="'. __('Read', 'masterwp') . get_the_title($post->ID).'">'. __('... Read more &raquo;', 'masterwp') .'</a>';
}
...
function master_get_the_author_posts_link() {
	global $authordata;
	...
	$link = sprintf(
		...
		esc_attr( sprintf( __( 'Posts by %s', 'masterwp' ), get_the_author() ) ), // No further l10n needed, core will take care of this one
		get_the_author()
	);
	return $link;
}